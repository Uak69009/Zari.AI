...
export default function CropCards() {
  return (
    <section className="w-full py-20 bg-gray-50 text-gray-900 border-t border-gray-200">
      <div className="max-w-7xl mx-auto px-6">
        
        {/* Header Title */}
        <div className="mb-14">
          <div className="flex items-center gap-2 text-emerald-700 text-base font-semibold tracking-wider uppercase mb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {cropData.map((crop) => (
            <div
              key={crop.id}
              className="group bg-white border border-gray-200 hover:border-emerald-500 rounded-2xl p-7 transition-all duration-300 flex flex-col justify-between shadow-sm hover:shadow-xl relative overflow-hidden"
            >
              <div>
                {/* Card Top: Title Left, Realistic Crop Image Right */}
                <div className="flex items-start justify-between gap-5 mb-5">
                  <div className="flex-1">
                    <h3 className="text-2xl font-bold text-gray-900 group-hover:text-emerald-700 transition-colors leading-snug">
                      {crop.name}
                    </h3>
                    <span className="inline-block mt-2 text-xs md:text-sm font-semibold text-emerald-800 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full">
                      {crop.category}
                    </span>
                  </div>
                  
                  {/* Crop Image Thumbnail */}
                  <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl overflow-hidden border border-gray-200 flex-shrink-0 bg-gray-100 shadow-sm group-hover:border-emerald-400 transition-colors">
                ...
                {/* Info Description */}
                <p className="text-sm md:text-base text-gray-600 leading-relaxed mb-6 border-b border-gray-100 pb-5">
                  {crop.description}
                </p>
                ...
                <div className="space-y-2.5 mb-6">
                  ...
                  {crop.diseases.map((dis, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between text-xs md:text-sm bg-gray-50 px-3.5 py-2.5 rounded-xl border border-gray-200/70"
                    >
                      <div className="flex items-center gap-2.5">
                        <span className={`w-2 h-2 rounded-full ${dis.severity === "High" ? "bg-red-500" : "bg-amber-500"}`} />
                        <span className="text-gray-900 font-medium">{dis.name}</span>
                      </div>
                      <span className="text-gray-600 text-xs md:text-sm font-serif" dir="rtl">
                        {dis.urdu}
                      </span>
                    </div>
                  ))}
              ...
              {/* Card Footer Action */}
              <div className="pt-4 border-t border-gray-100 flex items-center justify-between text-sm text-emerald-700 group-hover:text-emerald-800 font-bold transition-colors">
